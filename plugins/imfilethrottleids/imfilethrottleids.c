...
MODULE_TYPE_NOKEEP
MODULE_CNFNAME("imfilethrottleids")

/* defines */
const char *policy = "/opt/logs/policy.csv";
...
static throttle_ids_t	*s_throttleIdsInstance = NULL;
static pthread_mutex_t  s_mutex_throttleIdsInstance = PTHREAD_MUTEX_INITIALIZER;
	 ...
	 const size_t lenbuf)
{
	...
	snprintf((char*)buf, lenbuf - 1, "imfilethrottleids-state:%s", inst->pszFileName_forOldStateFile);
...
wdmapAdd(int wd, act_obj_t *const act)
{
	...
	if(i >= 0 && wdmap[i].wd == wd) {
		LogError(0, RS_RET_INTERNAL_ERROR, "imfilethrottleids: wd %d already in wdmap!", wd);
...
in_setupWatch(act_obj_t *const act, const int is_file)
{
	...
	if(wd < 0) {
		LogError(errno, RS_RET_IO_ERROR, "imfilethrottleids: cannot watch object '%s'",
			act->name);
		goto done;
	}
	wdmapAdd(wd, act);
...
fen_setupWatch(act_obj_t *const act)
{
	...
	if(act->pfinf == NULL) {
		...
		if (act->pfinf == NULL) {
			LogError(errno, RS_RET_OUT_OF_MEMORY, "imfilethrottleids: fen_setupWatch alloc memory "
				"for fileinfo failed ");
			goto done;
		}
	...
	const ino_t ino)
{
	...
	if(is_file) {
		...
		CHKiRet(ratelimitNew(&act->ratelimiter, "imfilethrottleids", name));
		...
		for(unsigned i = 0 ; i < files.gl_pathc ; i++) {
			...
			if(stat(file, &fileInfo) != 0) {
				LogError(errno, RS_RET_ERR,
					"imfilethrottleids: poll_tree cannot stat file '%s' - ignored", file);
				continue;
			}
			...
			if(!is_file && S_ISREG(fileInfo.st_mode)) {
				LogMsg(0, RS_RET_ERR, LOG_WARNING,
					"imfilethrottleids: '%s' is neither a regular file nor a "
					"directory - ignored", file);
				continue;
			}
			if(chld->is_file != is_file) {
				LogMsg(0, RS_RET_ERR, LOG_WARNING,
					"imfilethrottleids: '%s' is %s but %s expected - ignored",
...
act_obj_destroy(act_obj_t *const act, const int is_deleted)
{
	...
	// TODO: destruct our objects here?
}
		 ...
		 const size_t lenbuf)
{
	...
	snprintf((char*)buf, lenbuf - 1, "imfilethrottleids-state:%lld", (long long) act->ino);
	...
	const int64 strtOffs)
{
	...
	if (pthread_mutex_lock(&s_mutex_throttleIdsInstance)) {
		dbgprintf("s_mutex_throttleIdsInstance lock has failed!\n");
		assert(0);
		ABORT_FINALIZE(RS_RET_ERR);
	}
	throttleIdsAddMsg(s_throttleIdsInstance, pMsg, NULL);

	pthread_mutex_unlock(&s_mutex_throttleIdsInstance);
...
openFileWithStateFile(act_obj_t *const act)
{
	...
	if(fd < 0) {
		if(errno == ENOENT) {
			...
			LogError(errno, RS_RET_IO_ERROR,
				"imfilethrottleids error trying to access state file for '%s'",
			        act->name);
			ABORT_FINALIZE(RS_RET_IO_ERROR);
		}
	}
	...
	if(json == NULL) {
		LogError(0, RS_RET_ERR, "imfilethrottleids: error reading state file for '%s'", act->name);
	}
...
checkInstance(instanceConf_t *const inst)
{
	...
	if(loadModConf->normalizePath) {
		if(inst->pszFileName[0] == '.' && inst->pszFileName[1] == '/') {
			DBGPRINTF("imfilethrottleids: removing heading './' from name '%s'\n", inst->pszFileName);
		...
		if(inst->pszFileName[0] != '/') {
			if(getcwd((char*)curr_wd, MAXFNAME) == NULL || curr_wd[0] != '/') {
				LogError(errno, RS_RET_ERR, "imfilethrottleids: error querying current working "
			...
			if(len_curr_wd + ustrlen(inst->pszFileName) + 1 >= MAXFNAME) {
				LogError(0, RS_RET_ERR, "imfilethrottleids: length of configured file and current "
	...
	dbgprintf("imfilethrottleids: adding file monitor for '%s'\n", inst->pszFileName);
...
addInstance(void __attribute__((unused)) *pVal, uchar *pNewVal)
{
	...
	if(cs.pszFileName == NULL) {
		LogError(0, RS_RET_CONFIG_ERROR, "imfilethrottleids error: no file name given, file monitor can "
					"not be created");
		ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
	}
	if(cs.pszFileTag == NULL) {
		LogError(0, RS_RET_CONFIG_ERROR, "imfilethrottleids error: no tag value given, file monitor can "
					"not be created");
		ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
	}
...
BEGINnewInpInst
	...
	int i;
CODESTARTnewInpInst
	DBGPRINTF("newInpInst (imfilethrottleids)\n");
	...
	if(Debug) {
		DBGPRINTF("input param blk in imfilethrottleids:\n");
	...
	if(inst->startRegex != NULL) {
		...
		if(errcode != 0) {
			...
			parser_errmsg("imfilethrottleids: error in regex expansion: %s", errbuff);
			ABORT_FINALIZE(RS_RET_ERR);
		}
	}
...
CODESTARTsetModCnf
	...
	if(pvals == NULL) {
		LogError(0, RS_RET_MISSING_CNFPARAMS, "imfilethrottleids: error processing module "
				"config parameters [module(...)]");
		ABORT_FINALIZE(RS_RET_MISSING_CNFPARAMS);
	}

	if(Debug) {
		DBGPRINTF("module (global) param blk for imfilethrottleids:\n");
	...
	for(i = 0 ; i < modpblk.nParams ; ++i) {
		...
		if(!strcmp(modpblk.descr[i].name, "pollinginterval")) {
			...
			if(!es_strconstcmp(pvals[i].val.d.estr, "polling"))
				...
				LogError(0, RS_RET_PARAM_ERROR, "imfilethrottleids: unknown "
					"mode '%s'", cstr);
				free(cstr);
			}
		} else {
...
CODESTARTcheckCnf
	...
	if(pModConf->root == NULL) {
		LogError(0, RS_RET_NO_LISTNERS,
				"imfilethrottleids: no files configured to be monitored - "
...
CODESTARTactivateCnf
	...
	if(runModConf->root == NULL) {
		LogError(0, NO_ERRCODE, "imfilethrottleids: no file monitors configured, "
				"input not activated.\n");
	...
	for(inst = runModConf->root ; inst != NULL ; inst = inst->next) {
		...
		if(!containsGlobWildcard((char*)inst->pszFileName)) {
			if(access((char*)inst->pszFileName, R_OK) != 0) {
				LogError(errno, RS_RET_ERR,
					"imfilethrottleids: on startup file '%s' does not exist "
...
{
	if(ev->mask & IN_IGNORED) {
		DBGPRINTF("imfilethrottleids: got IN_IGNORED event\n");
		goto done;
	}
	...
	if(etry == NULL) {
		LogMsg(0, RS_RET_INTERNAL_ERROR, LOG_WARNING, "imfilethrottleids: internal error? "
...
do_inotify(void)
{
	...
	ino_fd = inotify_init();
	if(ino_fd < 0) {
		LogError(errno, RS_RET_INOTIFY_INIT_FAILED, "imfilethrottleids: Init inotify "
			"instance failed ");
	...
	while(glbl.GetGlobalInputTermState() == 0) {
		...
		if(rd < 0) {
			LogError(errno, RS_RET_IO_ERROR, "imfilethrottleids: error during inotify - ignored");
			continue;
		}
		currev = 0;
		while(currev < rd) {
...
do_inotify(void)
{
	LogError(0, RS_RET_NOT_IMPLEMENTED, "imfilethrottleids: mode set to inotify, but the "
...
CODESTARTrunInput
	...
	if(runModConf->opMode == OPMODE_POLLING)
		...
		LogError(0, RS_RET_NOT_IMPLEMENTED, "imfilethrottleids: unknown mode %d set",
			runModConf->opMode);
		return RS_RET_NOT_IMPLEMENTED;
	}
...
CODESTARTwillRun
	...
	CHKiRet(prop.SetString(pInputName, UCHAR_CONSTANT("imfilethrottleids"), sizeof("imfilethrottleids") - 1));
...
atomicWriteStateFile(const char *fn, const char *content)
{
	...
	if(fd < 0) {
		LogError(errno, RS_RET_IO_ERROR, "imfilethrottleids: cannot open state file '%s' for "
	...
	if(w != (ssize_t) toWrite) {
		LogError(errno, RS_RET_IO_ERROR, "imfilethrottleids: partial write to state file '%s' "
...
finalize_it:
	if(iRet != RS_RET_OK) {
		LogError(0, iRet, "imfilethrottleids: could not persist state "
...
CODESTARTmodExit
	...
	throttleIdsDestruct(s_throttleIdsInstance);
...
{
	LogError(0, NO_ERRCODE, "imfilethrottleids: ruleset '%s' for %s not found - "
...
CODEmodInit_QueryRegCFSLineHdlr
	...
	// TODO: figure out if we need to do this on a per input module basis
	// use Throttle globally for now, since the policy is module specific
	CHKiRet(throttleIdsNew(&s_throttleIdsInstance, policy));
